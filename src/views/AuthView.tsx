...
export const AuthView: React.FC = () => {
  ...
  const handleQuickTouristLogin = () => {
    login({
      id: 'usr-tourist-01',
      name: 'Rian Prasetya (Wisatawan)',
      email: 'rian.tourist@example.com',
      role: 'wisatawan',
      avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&w=150&q=80'
    });
    navigateTo('home');
  ...
  const handleQuickSellerLogin = () => {
    login({
      id: 'usr-seller-01',
      name: 'I Wayan Suparta (Pengelola Desa)',
      email: 'wayan.penglipuran@example.com',
      role: 'penjual',
      avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80',
      sellerName: 'Koperasi BUMDes Penglipuran'
    });
      ...
      <div className="bg-white p-6 sm:p-8 rounded-3xl border border-stone-200 shadow-xl text-center space-y-6">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-stone-500">
            Akses marketplace desa wisata sebagai wisatawan atau pengelola produk desa.
          </p>
        </div>
        ...
        <div className="p-4 bg-amber-50 rounded-2xl border border-amber-200/80 space-y-3 text-left">
          <span className="text-[11px] font-bold text-amber-900 uppercase tracking-wider block">
            ⚡ Masuk Cepat Demo UTS:
          </span>
          
          <div className="space-y-2">
            <button
              onClick={handleQuickTouristLogin}
              className="w-full py-2.5 px-3 bg-white hover:bg-stone-50 border border-stone-300 rounded-xl text-xs font-bold text-stone-800 flex items-center justify-between transition-colors shadow-xs"
            >
            ...
            <button
              ...
              className="w-full py-2.5 px-3 bg-emerald-800 hover:bg-emerald-900 text-amber-200 rounded-xl text-xs font-bold flex items-center justify-between transition-colors shadow-xs"
            >
              <div className="flex items-center gap-2">
                <Store className="w-4 h-4 text-amber-300" />
                <span>Masuk sebagai Pengelola Desa</span>
              </div>
